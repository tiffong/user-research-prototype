...
def parse_req():
    ...
    result["circle"] = requestedData["circle"]
    result["square"] = requestedData["square"]
    result["triangle"] = requestedData["triangle"]
    result["bright_dark"] = requestedData["bright_dark"]
    result["soft_sharp"] = requestedData["soft_sharp"]
    result["warm_cool"] = requestedData["warm_cool"]
    result["simple_complex"] = requestedData["simple_complex"]
    result["disorder_inorder"] = requestedData["disorder_inorder"]
    result["high_low"] = requestedData["high_low"]
...
def parse_req_2():
    ...
    result["circle_1"] = requestedData["circle_1"]
    result["square_1"] = requestedData["square_1"]
    result["triangle_1"] = requestedData["triangle_1"]
    result["bright_dark_1"] = requestedData["bright_dark_1"]
    result["soft_sharp_1"] = requestedData["soft_sharp_1"]
    result["warm_cool_1"] = requestedData["warm_cool_1"]
    result["simple_complex_1"] = requestedData["simple_complex_1"]
    result["disorder_inorder_1"] = requestedData["disorder_inorder_1"]
    result["high_low_1"] = requestedData["high_low_1"]
    result["random_noise_1"] = requestedData["random_noise_1"]
    ...
    result["circle_2"] = requestedData["circle_2"]
    result["square_2"] = requestedData["square_2"]
    result["triangle_2"] = requestedData["triangle_2"]
    result["bright_dark_2"] = requestedData["bright_dark_2"]
    result["soft_sharp_2"] = requestedData["soft_sharp_2"]
    result["warm_cool_2"] = requestedData["warm_cool_2"]
    result["simple_complex_2"] = requestedData["simple_complex_2"]
    result["disorder_inorder_2"] = requestedData["disorder_inorder_2"]
    result["high_low_2"] = requestedData["high_low_2"]
    result["random_noise_2"] = requestedData["random_noise_2"]
...
def img_augmentation():     
    ...
    with tf.Session() as sess:
        ...
        for i in range(lp):
            if i < 3:
                condition_change = copy.deepcopy(condition_input)
                print("before change:{}".format(condition_change[0]))
                ...
                print("after change:{}".format(condition_change[0]))
        ...
        for i in range(lp):
            if i < 3:
                ...
                condition_change = copy.deepcopy(condition_input)
                for j in range(len(condition_change)):
                    condition_change[j][7] = condition_change[j][7] - (condition_change[j][7] - 0)*(i-2) / 3
                condition_final_output.append(condition_change[0])
                pic = sess.run(g_output, feed_dict = {noise_img: noise_input, condition_img: condition_change})
                pic = [pic[0]]
                img = np.vstack((img, pic))
